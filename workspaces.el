...
(defun workspaces-open (&optional project)
  ...
  (let* ((project-switch-commands workspaces-project-switch-commands)
         ...
         (tab-name (workspaces--generate-name project-dir existed-workspaces))
         (dir-potential-project (project--find-in-directory project-dir))
         (tab-existed-p (member (workspaces--current-name) existed-workspaces))
         (project-remembered-p (member (list project-dir) project--list)))

    (message "[workspaces] Project directory: %s" project-dir)
