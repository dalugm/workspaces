...
(defcustom workspaces-keymap-prefix (kbd "C-c C-w")
...
;;;###autoload
(defun workspaces--current-tab-name ()
    ...
    (cond
     ...
     (t
      (message (format "Buffer `%s' removed from `%s' workspace."
                       buffer (workspaces--current-tab-name)))))
    (bury-buffer buffer)
...
(defun workspaces--generate-tab-name (base-name)
  "Generate a unique numbered tab name."
  (let ((counter 2)
        (new-name base-name))
    (while (member base-name (workspaces--list))
      (setq new-name (format "%s<%d>" base-name counter)
            counter (+ counter 1)))
    new-name))

;;;###autoload
(defun workspaces-open (&optional project)
  ...
  (let* ((project-switch-commands workspaces-project-switch-commands)
    ...
    (cond
     ...
     ;; exists while tab not, then open.
...
;;;; Define Keymaps
(defvar workspaces-mode-map
  (let ((prefix-map (make-sparse-keymap))
        (map (make-sparse-keymap)))
    (define-key prefix-map (kbd "b")   #'workspaces-switch-to-buffer)
    (define-key prefix-map (kbd "C-b") #'workspaces-switch-to-buffer)
    (define-key prefix-map (kbd "c")   #'workspaces-clear-buffers)
    (define-key prefix-map (kbd "C-c") #'workspaces-clear-buffers)
    (define-key prefix-map (kbd "f")   #'workspaces-forget-workspace)
    (define-key prefix-map (kbd "C-f") #'workspaces-forget-workspace)
    (define-key prefix-map (kbd "k")   #'workspaces-kill-buffer)
    (define-key prefix-map (kbd "C-k") #'workspaces-kill-buffer)
    (define-key prefix-map (kbd "l")   #'workspaces-switch)
    (define-key prefix-map (kbd "C-l") #'workspaces-switch)
    (define-key prefix-map (kbd "n")   #'workspaces-rename)
    (define-key prefix-map (kbd "C-n") #'workspaces-rename)
    (define-key prefix-map (kbd "o")   #'workspaces-open)
    (define-key prefix-map (kbd "C-o") #'workspaces-open)
    (define-key prefix-map (kbd "q")   #'workspaces-close)
    (define-key prefix-map (kbd "C-q") #'workspaces-close)
    (define-key prefix-map (kbd "s")   #'workspaces-switch)
    (define-key prefix-map (kbd "C-s") #'workspaces-switch)
    (define-key prefix-map (kbd "t")   #'workspaces-switch-buffer-and-tab)
    (define-key prefix-map (kbd "C-t") #'workspaces-switch-buffer-and-tab)
    (define-key prefix-map (kbd "z")   #'workspaces-forget-zombie)
    (define-key prefix-map (kbd "C-z") #'workspaces-forget-zombie)
    (define-key map workspaces-keymap-prefix prefix-map)
    map)
  "Keymap for Workspaces commands.")

;;;###autoload
(define-minor-mode workspaces-mode
  "Minor mode for buffer-isolated workspaces."
  :keymap workspaces-mode-map
  :global t
  (if workspaces-mode
      (progn
